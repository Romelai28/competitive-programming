struct SqrtDescomposition{
	...
	int findBlock(int index){ return index/sqrtOfN;}
	int findIndexInTheBlock(int index){ return index % sqrtOfN;}
	...
	void answerQuery(int l, int r){
		...
		if (blockOfL == blockOfR){ forsn(i, indexOfL, indexOfR+1) // Proceso el intervalo del bloque particular
		} else {
